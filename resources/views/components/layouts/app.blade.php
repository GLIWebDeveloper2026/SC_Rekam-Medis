...
<html lang="id">
    ...
    <body class="antialiased">
        <div class="min-h-screen lg:grid lg:grid-cols-[17rem_1fr]" x-data="navigation">
            <aside class="fixed inset-y-0 left-0 z-50 flex w-72 -translate-x-full flex-col border-r border-slate-200 bg-white px-4 py-5 transition lg:static lg:w-auto lg:translate-x-0" :class="open ? 'translate-x-0' : '-translate-x-full'">
                <div class="flex items-center justify-between gap-3 px-2"><a class="flex items-center gap-3" href="{{ route('dashboard') }}"><span class="grid size-11 place-items-center rounded-xl bg-clinic-500 text-white"><i data-lucide="heart-pulse" class="size-6"></i></span><span><strong class="block font-heading text-base text-clinic-800">Sehat Bersama</strong><span class="block text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400">Sistem Klinik</span></span></a><button class="grid size-10 place-items-center lg:hidden" type="button" @click="close"><i data-lucide="x" class="size-5"></i></button></div>
                ...
                <div class="border-t border-slate-100 pt-4"><p class="px-3 text-sm font-bold text-slate-700">{{ auth()->user()->name }}</p><p class="px-3 text-xs text-slate-400">{{ auth()->user()->activeRoleCode() ?? 'staf' }}</p><form class="mt-3" method="POST" action="{{ route('logout') }}">@csrf<button class="flex min-h-11 w-full items-center gap-3 rounded-xl px-3 text-sm font-bold text-slate-500 hover:bg-danger/8 hover:text-danger" type="submit"><i data-lucide="log-out" class="size-5"></i> Keluar</button></form></div>
            </aside>
            <div class="min-w-0"><header class="sticky top-0 z-30 flex min-h-18 items-center justify-between border-b border-slate-200/80 bg-white/90 px-4 backdrop-blur sm:px-6 lg:px-8"><button class="grid size-11 place-items-center rounded-xl border border-slate-200 lg:hidden" type="button" @click="toggle"><i data-lucide="menu" class="size-5"></i><span class="sr-only">Menu</span></button><div class="ml-auto text-right"><p class="text-xs font-bold uppercase tracking-[0.16em] text-clinic-600">Waktu klinik</p><p class="text-sm font-bold text-slate-700">{{ now()->format('d-m-Y H:i:s') }} WIB</p></div></header><main class="px-4 py-8 sm:px-6 lg:px-8">{{ $slot }}</main></div>
